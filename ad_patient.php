                            ...
                            <div class="col-md-12">
                                ...
                                <div class="table-responsive m-b-40">
                                    <table class="table table-borderless table-data3">
                                        ...
                                        <tbody>
                                            <?php
                                            $sql = "SELECT patient_name, patient_email, password, gender, bdate, address, phoneno FROM patient";
                                            if ($result = $conn->query($sql)) {
                                                if ($result->num_rows >= 1) {
                                                    // output data of each row
                                                    while($row = $result->fetch_assoc()) {
                                                        echo "<tr><td>" . $row["patient_name"]. "</td><td>" . $row["patient_email"]. "</td><td>" . $row["password"]. "</td><td>" . $row["phoneno"]. "</td><td>" . $row["address"]. "</td><td><button onclick='view(\"ad_patient_view_email\", \"" . $row["patient_email"]. "\", \"ad_patient_view.php\")' class='btn btn-block btn-danger text-white'>View</button></td></tr>";
                                                    }
                                                }
                                            } else {
                                                echo "Query Failed";
