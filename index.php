...
<body>
	...
	<?php
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			if (empty($_POST["log_password"])) {
			  $log_password_err = "Password is required";
			} else {
			  $log_password = test_input($_POST["log_password"]);
			  $log_password_err = "";
			  $done = $done + 1;
			}
		...
		function test_input($data) {
			...
			if($done == 2) {
				if($login_mode == "admin") {
					$sql = "select password from admin where ad_email = '". $log_email . "'";
					...
					if ($result->num_rows > 0) {
						...
						if($row['password'] == $log_password) {
							...
							echo "<script>window.location = 'ad_profile.php';</script>";
						} else {
				...
				if($login_mode == "doctor") {
					$sql = "select password from doctor where doc_email = '". $log_email . "'";
					$result = $conn->query($sql);
					if ($result->num_rows > 0) {
						// output data of each row
						$row = $result->fetch_assoc();
				...
				if($login_mode == "patient") {
					$sql = "select password from patient where patient_email = '". $log_email . "'";
					$result = $conn->query($sql);
					if ($result->num_rows > 0) {
						// output data of each row
						$row = $result->fetch_assoc();
								...
								<nav class="navbar navbar-expand-lg">
									...
									<div class="collapse navbar-collapse sub-menu-bar" id="navbarSupportedContent">
										<ul id="nav" class="navbar-nav ml-auto">
                                            ...
                                            <li class="nav-item active">
												<a class="page-scroll" href="patient_signup.php">SignUp</a>
                                            </li>
                                            <li class="nav-item active">
												<a class="page-scroll" href="#login">Request Bed</a>
											</li>
										</ul>
									</div> <!-- navbar collapse -->
								</nav> <!-- navbar -->
	...
	<section class="we-do-section pt-150">
		<div class="shape shape-1">
			<img src="homepage_templete/assets/img/shapes/shape-1.svg" alt="">
		</div>
		<div class="container">
			<div class="row">
				<div class="col-xl-8 mx-auto">
					<div class="section-title text-center mb-55">
						<span class="wow fadeInDown" data-wow-delay=".2s">What does <b>Hospitobed</b> Do</span>
						<h2 class="mb-15 wow fadeInUp" data-wow-delay=".4s"> We Care For Your Health Every Moment</h2>
						<p class="wow fadeInUp" data-wow-delay=".6s">we can help you by providing conditions and also availability of beds as well as you can make reservation of beds <br class="d-none d-lg-block"> We Care For Your Health Every Moment.</p>
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-xl-12">
					<div class="graph mb-60">
						<img src="homepage_templete/assets/img/we-do/graph-img.svg" alt="">
					</div>
				</div>
				<div class="col-lg-3">
				...
				<div class="col-lg-3">
					<div class="we-do-item mb-30">
						<div class="we-do-icon mb-25">
							<svg  width="53.947" height="42.07" viewBox="0 0 53.947 42.07">
                                ...
                                <div class="card" style=:"align:center">
                                    <div class="card-body">
                                        <div class="card-title">
                                            <h3 class="text-center title-2">sign in</h3>
                                        </div>
                                        <hr>
										...
										<p class="error"><?php echo $login_err; ?></p>
                                            <div class="form-group">
                                                ...
                                                <input id="cc-pament" name="log_email" value="<?php echo $log_email;?>" type="text" class="form-control" aria-required="true" aria-invalid="false">
		  										<p class="error"><?php echo $log_email_err; ?></p>
											</div>
                                            <div class="form-group has-success">
                                                <label for="cc-name" class="control-label mb-1">Password</label>
                                                <input id="cc-name" name="log_password" value="<?php echo $log_password;?>" type="password" class="form-control cc-name valid" data-val="true" data-val-required="Please enter the name on card"
                                                    autocomplete="cc-name" aria-required="true" aria-invalid="false" aria-describedby="cc-name-error">
												<p class="error"><?php echo $log_password_err; ?></p>
												<span class="help-block field-validation-valid" data-valmsg-for="cc-name" data-valmsg-replace="true"></span>
                                            </div>
                                            <div class="row form-group">
                                                <div class="col col-md-3">
                                                    <label class=" form-control-label">Login Mode</label>
                                                </div>
                                                <div class="col col-md-9">
                                                    <div class="form-check">
                                                        <div class="radio">
                                                            <label for="radio1" class="form-check-label ">
                                                                <input type="radio" id="radio1" name="login_mode" value="admin" class="form-check-input" checked>Admin
                                                            </label>
                                                        </div>
                                                        <div class="radio">
                                                            <label for="radio2" class="form-check-label ">
                                                                <input type="radio" id="radio2" name="login_mode" value="doctor" class="form-check-input">Doctor
                                                            </label>
                                                        </div>
                                                        <div class="radio">
                                                            <label for="radio3" class="form-check-label ">
                                                                <input type="radio" id="radio3" name="login_mode" value="patient" class="form-check-input">Patient
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>
