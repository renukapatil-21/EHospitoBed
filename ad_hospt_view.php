...
<?php
        ...
        $done = 0;
        $test = "";
        $hospt_id = $_SESSION['ad_hospt_view_id'];
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			if (empty($_POST["dist"])) {
                ...
                $dist_err = "";
                $done = $done + 1;
                // echo "<script> alert('9 ".$done."'); </script>";
              }
            // 10
			if (empty($_POST["hospt_name"])) {
				...
				$hospt_name_err = "";
				$done = $done + 1;
                // echo "<script> alert('10 ".$done."'); </script>";
              }
            // 11
			if (empty($_POST["address"])) {
				...
				$address_err = "";
				$done = $done + 1;
                // echo "<script> alert('11 ".$done."'); </script>";
			  }
			...
			// 12
			  if (empty($_POST["PhoneNo"])) {
				...
				$Phoneno_err = "";
				$done = $done + 1;
                // echo "<script> alert('12 ".$done."'); </script>";
              }
		}
		// echo $done;
