...
<?php
		...
		$login_err = $login_mode = "";
		$done = 0;
        $sql = "select * from patient where patient_email='".$_SESSION['doc_patient_view_email']."'";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			// output data of each row
			$row = $result->fetch_assoc();
		...
		function test_input($data) {
			...
			if($done == 5) {
				// echo "hey";
				$sql = "update patient set password = '".$password."', patient_name = '".$patient_name."', gender = '".$Gender."', bdate = '".$Bdate."', address = '".$Address."', phoneno = ".$PhoneNo." where patient_email = '". $email ."'";
				if ($conn->query($sql) === TRUE) {
					echo "<script>window.location = 'ad_patient_view.php';</script>";
				} else {
                                ...
                                <div class="card" style=:"align:center">
                                    <div class="card-body">
                                        <div class="card-title">
                                            <h3 class="text-center title-2">Patient Profile</h3>
                                        </div>
                                        <hr>
                                                    ...
                                                    <div class="form-check">
                                                        <div class="radio">
                                                            <label for="radio1" class="form-check-label ">
                                                                <input type="radio" id="radio1" name="Gender" value="Male" class="form-check-input" <?php if($Gender == "Male") {echo "checked";} ?> disabled>Male
                                                            </label>
                                                        </div>
                                                        <div class="radio">
                                                            <label for="radio2" class="form-check-label ">
                                                                <input type="radio" id="radio2" name="Gender" value="Female" class="form-check-input" <?php if($Gender == "Female") {echo "checked";} ?> disabled>Female
                                                            </label>
                                                        </div>
                                                        <div class="radio">
                                                            <label for="radio3" class="form-check-label ">
                                                                <input type="radio" id="radio3" name="Gender" value="Other" class="form-check-input" <?php if($Gender == "Other") {echo "checked";} ?> disabled>Other
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>
