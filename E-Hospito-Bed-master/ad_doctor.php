                                    ...
                                    <table class="table table-borderless table-data3">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Phone no</th>
                                                <th>Hospt Id</th>
                                                <th><button onclick="add_dept_handler()" target="_blank" class="btn btn-success  d-none d-md-block pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light">Add Doctor</button></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
                                            $sql = "SELECT doc_name, doc_email, password, gender, bdate, address, phoneno, hospt_id FROM doctor";
                                            if ($result = $conn->query($sql)) {
                                                if ($result->num_rows >= 1) {
                                                    // output data of each row
                                                    while($row = $result->fetch_assoc()) {
                                                        echo "<tr><td>" . $row["doc_name"]. "</td><td>" . $row["doc_email"]. "</td><td>" . $row["password"]. "</td><td>" . $row["phoneno"]. "</td><td>" . $row["hospt_id"]. "</td><td><button onclick='view(\"ad_doctor_view_email\", \"" . $row["doc_email"]. "\", \"ad_doctor_view.php\")' class='btn btn-block btn-danger text-white'>View</button></td></tr>";
                                                    }
                                                }
                                            } else {
                                                echo "Query Failed";
