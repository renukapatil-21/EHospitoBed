...
<?php
		...
		function test_input($data) {
			...
			if($done == 6) {
				// echo "hey";
				$sql = "select doc_name from doctor where doc_email = '". $email . "'";
					$result = $conn->query($sql);
					if ($result->num_rows == 0) {
						// output data of each row
						$sql = "insert into doctor values('".$email."', '".$password."', '".$patient_name."', '".$Gender."', '".$Bdate."', '".$Address."', ".$PhoneNo.", ".$hospt_id.")";
						if ($conn->query($sql) === TRUE) {
                            $_SESSION['ad_doctor_view_email'] = $email;
